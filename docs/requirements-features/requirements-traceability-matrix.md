...
| GR-15, GR-16 | MVC view decomposition, separation of concerns | libGDX `GdxStartMenuView`, `GdxHudView`, `GdxGameWorldView`, `GdxOverlayView` orchestrated by `GdxGameScreen.draw()` | `GdxGameScreenLayoutTest`, `GdxGameScreenParityTest` |
...
| SR-22 (partial) | Shared session aggregate, single source of runtime mode state | `GameSession`, `GameMode`, libGDX `GdxGameScreen` mode and session flag migration | `GdxGameScreenParityTest`, `GdxGameScreenLayoutTest`, `GdxGameScreenTerminalCommandTest` |
...
| SR-24 (proposed) | Adapter pattern, incremental migration | libGDX `GdxGame` + `ApplicationAdapterScreen` route existing `GdxGameScreen` through `Game.setScreen(...)` | `ApplicationAdapterScreenTest.showInitializesDelegateOnlyOnce`, `ApplicationAdapterScreenTest.renderAndResizeDelegateToApplicationAdapter`, `ApplicationAdapterScreenTest.disposeDelegatesToApplicationAdapter` |
| SR-25 (proposed) | Repository boundary around assets, SRP | libGDX `GdxAssetService` centralizes texture path normalization and AssetManager ownership | `GdxAssetServiceTest.normalizeInternalPathRemovesLeadingSlash`, `GdxAssetServiceTest.normalizeInternalPathKeepsRelativePath`, `GdxAssetServiceTest.normalizeInternalPathReturnsNullForBlankInput` |
