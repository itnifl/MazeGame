...
This module holds the Acceleo templates and a small headless launcher used to generate Java source code for the game. It reads your EMF model and writes the produced sources into the `maze-generated` module so that the app and tests can depend on a regular jar.
...
* **What it does:**
  ...
  4. Invokes Acceleo to generate Java files into `maze-generated/src/main/java`.
...
* **How to run it:** Run as a normal Java application and pass two arguments:
  ...
  `args[1]` — output source directory (usually `maze-generated/src/main/java`)
...
* Generated Java sources: `maze-generated/src/main/java`
  These are compiled and packaged by the `maze-generated` module into a jar that `main.game.maze` depends on.
...
4. Files are written to `maze-generated/src/main/java`.
5. `maze-generated` compiles and publishes a jar consumed by `main.game.maze` and tests.
...
* If you change templates, re-run the generator (Tycho or `HeadlessGeneratorApp`) so the `maze-generated` sources are updated before compiling the app.
...
* Keep the output directory (`maze-generated/src/main/java`) under version control policy that fits your workflow. Many teams commit generated sources for simpler IDE import, but you can also ignore and always regenerate in CI.
